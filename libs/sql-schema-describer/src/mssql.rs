...
impl SqlSchemaDescriber {
    ...
    async fn get_table_names(&self, schema: &str) -> Vec<String> {
        ...
        let select = Select::from_table(("information_schema", "tables"))
            .so_that("table_type".equals("BASE TABLE"))
            .and_where("table_schema".equals(schema))
            .order_by("table_name");

        let rows = self.conn.select(select).await.expect("get table names");
