...
pub(crate) fn render_input_type<'a>(input_type: &InputType, ctx: &mut RenderContext<'a>) -> DMMFTypeInfo {
    match input_type {
        InputType::Object(ref obj) => {
            ctx.to_be_rendered.push(obj.into());
        ...
        InputType::Enum(et) => {
            ctx.to_be_rendered.push(et.into());
            let type_info = DMMFTypeInfo {
...
// WIP dedup code
pub(crate) fn render_output_type<'a>(output_type: &'a OutputType, ctx: &mut RenderContext<'a>) -> DMMFTypeInfo {
    match output_type {
